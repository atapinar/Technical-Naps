...
SYMBOLS_TO_ANALYZE_STR = os.getenv('SYMBOLS_TO_ANALYZE', 'TSLA')
...
# Add or update the main function to only run on weekdays
async def main():
    """
    Main function to setup and run the scheduler.
    The job will only run on weekdays (Monday to Friday).
    """
    scheduler = AsyncIOScheduler()
    
    # Add job with day_of_week restriction (0-4 represents Monday to Friday)
    scheduler.add_job(
        send_telegram_report_job,
        CronTrigger(hour=21, minute=0, day_of_week='0-4'),  # Run at 9:00 PM on weekdays only
        id='telegram_report_job',
        name='Send Telegram Reports',
        replace_existing=True
    )
    
    logging.info("Starting scheduler - reports will run at 9:00 PM on weekdays only (Monday to Friday)")
    scheduler.start()
